  ...
  try {
    await getCotcoData()
      .then(async (res) => {
        ...
        if (dataLenght > 0) {
          const data = res.utilDataTrips.sort((a, b) =>
          ...
          const replaceEmptyCol = data.map((item) => {
            ...
            if (
              !item['HOROTAGE DEBUT'] ||
              item['HOROTAGE DEBUT'] === null ||
              item['HOROTAGE DEBUT'] === undefined
            ) {
              return {
              ...
              !item['LOCALISATION DE DEBUT'] ||
              item['LOCALISATION DE DEBUT'] === null ||
              item['LOCALISATION DE DEBUT'] === undefined
            ) {
              return {
                VEHICULE: item.VEHICULE,
              ...
              !item['HORODATAGE FIN'] ||
              item['HORODATAGE FIN'] === null ||
              item['HORODATAGE FIN'] === undefined
            ) {
              return {
                VEHICULE: item.VEHICULE,
                'HORODATAGE DEBUT': formatHD,
              ...
              !item['LOCALISATION DE FIN'] ||
              item['LOCALISATION DE FIN'] === null ||
              item['LOCALISATION DE FIN'] === undefined
            ) {
              return {
                VEHICULE: item.VEHICULE,
                'HORODATAGE DEBUT': formatHD,
          ...
          const groupByVehicle = _.groupBy(
            replaceEmptyCol,
            (item) => item.VEHICULE
          );
...
async function generateAllRepportCotco() {
  //wait generateDaylyRepportCotco();
  cron.schedule(
    '30 6 * * *',
    async () => {
