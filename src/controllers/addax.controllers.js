      ...
      await getRepportData(
        ...
        .then(async (res) => {
          ...
          if (objLenth > 0) {
            ...
            console.log(
              `no data found in ${EXCEPTION_REPORT_VEHICULES_ADDAX_PETROLEUM} ${ECO_CONDUITE}`
            );
          }
        })
      ...
      await getRepportData(
        ...
        .then(async (res) => {
          ...
          if (objLenth > 0) {
            ...
            console.log(
              `no data found in ${EXCEPTION_REPORT_VEHICULES_ADDAX_PETROLEUM} ${EXCES_DE_VITESSE}`
            );
          }
        })
        .catch((err) => console.log(err));
      ...
      if (sender && receivers) {
        setTimeout(() => {
          sendMail(
            ...
            EXCEPTION_REPORT_VEHICULES_ADDAX_PETROLEUM,
            `${EXCEPTION_REPORT_SUBJECT_MAIL}`,
            `${EXCEPTION_REPORT_VEHICULES_ADDAX_PETROLEUM}.xlsx`,
  ...
  await getRepportData(
    ...
    .then(async (res) => {
      if (res?.obj?.length > 0) {
        ...
        console.log(
          `no data found in ${LIST_OF_VEHICLES_NOT_AT_ADDAX_PARKING} ${NOT_AT_PARKED}`
        );
      }
    })
    .then(() => {
  ...
  getRepportData(
    ...
    .then(async (res) => {
      ...
      if (objLenth > 0) {
        ...
        console.log(
          `no data found in ${ACTIVITY_REPORT_OF_ADDAX_PETROLEUM} ${SOMMAIRE}`
        );
      }
    })
      ...
      await getRepportData(
        ...
        .then(async (res) => {
          ...
          if (objLenth > 0) {
            ...
            console.log(
              `no data found in ${ACTIVITY_REPORT_OF_ADDAX_PETROLEUM} ${ACTIVITY_CARS}`
            );
          }
        })
        .catch((err) => console.log(err));
      ...
      await getRepportData(
        ...
        .then(async (res) => {
          ...
          if (objLenth > 0) {
            ...
            console.log(
              `no data found in ${ACTIVITY_REPORT_OF_ADDAX_PETROLEUM} ${EXCEPTIONS}`
            );
          }
        })
        .catch((err) => console.log(err));
...
async function AddaxMonthlyRepportSynthese() {
  ...
  const sommaireData = await getRepportData(
    ADMIN_ADDAX,
    ACTIVITY_REPORT_OF_ADDAX_PETROLEUM,
    ADDAX_PETROLEUM,
    firstDayMonth,
  ...
  const syntheseData = await getRepportData(
    ADMIN_ADDAX,
    SYNTHESE,
    ADDAX_PETROLEUM,
    firstDayMonth,
  ...
  if (sender && receivers) {
    setTimeout(() => {
      sendMail(
        ...
        `${ACTIVITY_REPORT_OF_ADDAX_PETROLEUM}`,
        `${ACTIVITY_REPORT_SUBJECT_MAIL}`,
        `${ACTIVITY_REPORT_SUBJECT_MAIL}.xlsx`,
        path.join(__dirname, `../../${pathFile}-${reportTitleDate}.xlsx`)
      );
      deleteFile(
        path.join(__dirname, `../../${pathFile}-${reportTitleDate}.xlsx`)
      );
