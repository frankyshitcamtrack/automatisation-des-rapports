    ...
    await getRepportData(
      ...
      .then(async (res) => {
        ...
        if (objLenth > 0) {
          ...
          console.log(
            `no data found in ${RAPPORT_JOURNALIER_GUINESS} ${RAPPORT_GUINNESS}`
          );
        }
      })
      .then(() => {
        if (sender && receivers) {
          setTimeout(() => {
            sendMail(
              ...
              RAPPORT_JOURNALIER_GUINESS,
              `${ACTIVITY_REPORT_SUBJECT_MAIL_GUINNESS}`,
              `${RAPPORT_JOURNALIER_GUINESS}.xlsx`,
    ...
    await getRepportData(
      ...
      .then(async (res) => {
        ...
        if (objLenth > 0) {
          ...
          console.log(
            `no data found in ${RAPPORT_MENSUEL_GUINESS} ${EXCES_DE_VITESSE}`
          );
        }
      })
      .then(() => {
